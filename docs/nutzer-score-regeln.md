...
- `ProfileViewModel` berechnet den aktuellen Profilstand aus denselben Reviews und Reaktionen.
- `ProfileScreen` zeigt `totalScore`, Aktivität, Reputation und die Aktivitätsbestandteile im eigenen Profil nachvollziehbar an.
- Der provisorische Score-Chip auf dem Dashboard entfällt; der vorhandene Profilkreis führt stattdessen zur ausführlichen Profilansicht.
- Öffentliche Profile erhalten gemäß ADR-010 nur aggregierte Score- und Aktivitätswerte, aber keine chronologische Orts-Historie.
